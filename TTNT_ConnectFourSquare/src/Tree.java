...
public class Tree {
	...
	public Tree(Slot[][] slots, int depth) {
		...
		if (depth < maxDepth) {
			...
			for (int i = 0; i < nextMoves.size(); i++) {
				insertToSlot(slots[nextMoves.get(i)][0]); // Nhét thử quân vào ô

				Tree child = new Tree(slots, depth + 1); // Đệ quy cây tìm kiếm
															// (cây con)
				prev.setColorPiece(Piece.None);
	...
	public int nextConnect(int i, int j) {
		...
		value += connectedLinesOf4(i, j, -1, -1);
		value += connectedLinesOf4(i, j, -1, 0);
		value += connectedLinesOf4(i, j, -1, 1);
		value += connectedLinesOf4(i, j, 0, -1);
		value += connectedLinesOf4(i, j, 0, 1);
		value += connectedLinesOf4(i, j, 1, -1);
		value += connectedLinesOf4(i, j, 1, 0);
		value += connectedLinesOf4(i, j, 1, 1);
	...
	// Số hàng được 4 từ điểm xét
	public int connectedLinesOf4(int x, int y, int i, int j) {
		int value = 1;
